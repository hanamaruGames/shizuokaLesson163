...
bool CFbxMesh::Load(const TCHAR* FName)
{
	...
	{
		DWORD dwBufSize = GetFileSize(hFile, NULL);     // 入力ファイルのサイズを得る。
		BYTE* pBuf = new BYTE[dwBufSize];				  // 入力バッファを確保する 
		DWORD dwLen;
		ReadFile(hFile, pBuf, dwBufSize, &dwLen, NULL);		// ファイルをバッファに読み込む

		unsigned char* p = pBuf;	// バッファの先頭ポインタをセット
		if (p != nullptr) {
...
int CFbxMesh::GetEndFrame(const int& animID)
{
	...
	if (!m_Animation[animID].used)
	{
		MessageBox(nullptr, _T("■□■ GetEndFrame() ■□■"), _T("■□■ スキンメッシュに、指定されたアニメーションが読み込まれていません ■□■"), MB_OK);
		return 0;
	}
	return m_Animation[animID].endFrame - m_Animation[animID].startFrame;
}
...
//==========================================================================================================================================================
MATRIX4X4 CFbxMesh::GetFrameMatrices(int animID, int frame, DWORD nBone, DWORD nMesh)
{
	...
	mBoneWorld = m_pMeshArray[nMesh].m_BoneArray[animID][nBone].framePose[frame]; // -- 2018.8.28

	// ルートボーンアニメをするときの処理                                   // -- 2020.12.15
	if( m_RootAnimType[animID] != eRootAnimNone)
	{
		// ルートボーンアニメ行列の逆行列を掛け合わせてマトリックスを作成する
		mBoneWorld =
			mBoneWorld * XMMatrixInverse(nullptr, m_RootBoneArray[animID].framePose[frame]);
	}
...
MATRIX4X4 CFbxMesh::GetFrameMatrices(int animID, int frame, DWORD nBone)
{
	// メッシュ番号は０として呼び出し
	return GetFrameMatrices(animID, frame, nBone, 0);
}
...
//==========================================================================================================================================================
MATRIX4X4 CFbxMesh::GetFrameMatrices(int animID, int frame, const MATRIX4X4& mWorld, DWORD nBone, DWORD nMesh)
{
	MATRIX4X4 mBoneWorld;
...
MATRIX4X4 CFbxMesh::GetFrameMatrices(int animID, int frame, const MATRIX4X4& mWorld, DWORD nBone)
{
	// メッシュ番号は０として呼び出し
	return GetFrameMatrices(animID, frame, mWorld, nBone, 0);
}
...
//==========================================================================================================================================================
MATRIX4X4 CFbxMesh::GetRootAnimMatrices(int animID, int frame)
{
	MATRIX4X4 mBoneWorld;
	...
	mBoneWorld = m_RootBoneArray[animID].framePose[frame];
...
//==========================================================================================================================================================
MATRIX4X4 CFbxMesh::GetRootAnimUpMatrices(int animID, int frame, const int& StartFrameUp)
{
	...
	mBoneWorldOld = m_RootBoneArray[animID].framePose[frame-1];
...
void CFbxMesh::Render(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		...
		MessageBox(nullptr, _T("■□■ Render() ■□■"), _T("■□■ アニメーションの指定が必要です ■□■"), MB_OK);
		return;
	}
}
void CFbxMesh::RenderDisplace(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		...
		MessageBox(nullptr, _T("■□■ RenderDisplace() ■□■"), _T("■□■ スキンメッシュにアニメーションの指定が必要です ■□■"), MB_OK);
		return;
	}
}
...
void CFbxMesh::Render(int animID, int frame, const MATRIX4X4& mWorld)
{
	std::vector<ANIMATION_INFO> anim = { ANIMATION_INFO(animID, frame, 1.0f) };
	Render(anim, mWorld, GameDevice()->m_mView, GameDevice()->m_mProj, GameDevice()->m_vLightDir, GameDevice()->m_vEyePt);
}

void CFbxMesh::RenderDisplace(int animID, int frame, const MATRIX4X4& mWorld)
{
	std::vector<ANIMATION_INFO> anim = { ANIMATION_INFO(animID, frame, 1.0f) };
	RenderDisplace(anim, mWorld, GameDevice()->m_mView, GameDevice()->m_mProj, GameDevice()->m_vLightDir, GameDevice()->m_vEyePt);
}

void CFbxMesh::Render(std::vector<ANIMATION_INFO> anim, const MATRIX4X4& mWorld)
{
	Render(anim, mWorld, GameDevice()->m_mView, GameDevice()->m_mProj, GameDevice()->m_vLightDir, GameDevice()->m_vEyePt);
}

void CFbxMesh::RenderDisplace(std::vector<ANIMATION_INFO> anim, int frame, const MATRIX4X4& mWorld)
{
	RenderDisplace(anim, mWorld, GameDevice()->m_mView, GameDevice()->m_mProj, GameDevice()->m_vLightDir, GameDevice()->m_vEyePt);
}
...
//
//------------------------------------------------------------------------
void CFbxMesh::Render(std::vector<ANIMATION_INFO> anim, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		RenderStatic(mWorld, mView, mProj, vLight, vEye);
		...
		RenderSkin(anim, mWorld, mView, mProj, vLight, vEye);
	}
}

void CFbxMesh::RenderDisplace(std::vector<ANIMATION_INFO> anim, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		RenderDisplaceStatic(mWorld, mView, mProj, vLight, vEye);
		...
		RenderDisplaceSkin(anim, mWorld, mView, mProj, vLight, vEye);
	}
}

void CFbxMesh::MakeBoneMatrix(const std::vector<ANIMATION_INFO> anim, MATRIX4X4* mat, const CFbxMeshArray& _mesh)
{
	if (anim.size() == 1) {
		MATRIX4X4* org = _mesh.m_pBoneShader[anim[0].animID][anim[0].frame].shaderFramePose;
		for (int n = 0; n < _mesh.m_NumBones; n++) {
			*mat++ = *org++;
		}
	}
	else {
		...
		std::vector<ORGS> orgs;
		orgs.reserve(anim.size());
		for (auto a : anim) {
			ORGS org;
			org.rate = a.rate;
			org.ptr = (VECTOR4*)&_mesh.m_pBoneShader[a.animID][a.frame].shaderFramePose;
			orgs.push_back(org);
		}
		for (int n = 0; n < _mesh.m_NumBones * 4; n++) {
			VECTOR4 d = VECTOR4(0, 0, 0, 0);
			for (auto& org : orgs) {
				d += *org.ptr * org.rate;
				org.ptr++;
			}
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderSkin(const std::vector<ANIMATION_INFO> anim, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	// メッシュの描画
	Draw(anim, mWorld, mView, mProj, vLight, vEye);
}

void CFbxMesh::RenderDisplaceSkin(const std::vector<ANIMATION_INFO> anim, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	DrawDisplace(anim, mWorld, mView, mProj, vLight, vEye);
}
...
//
//------------------------------------------------------------------------
void CFbxMesh::Draw(const std::vector<ANIMATION_INFO> anim, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	...
	for (DWORD mi = 0; mi < m_dwMeshNum; mi++)   // -- 2018.8.1
	{
			...
			&pData)))                        // データの書き込み先ポインタ
		{
			...
			MakeBoneMatrix(anim, (MATRIX4X4*)pData.pData, m_pMeshArray[i]);
...
//
//------------------------------------------------------------------------
void CFbxMesh::DrawDisplace(const std::vector<ANIMATION_INFO> anim, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
