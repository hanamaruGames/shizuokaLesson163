...
#pragma clang diagnostic ignored "-Wzero-as-nullptr-pointer-constant"  // warning : zero as nullptr pointer constant              // some standard header variations use #define nullptr 0
#endif
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg)
{
    ...
    if (id != 0)
    {
        ...
        // We intentionally don't check if g.NavWindow != nullptr because g.NavAnyRequest should only be set when it is non nullptr.
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (g.NavWindowingList != nullptr && (window->Flags & ImGuiWindowFlags_NoNavFocus) == 0)   // Window titles visible when using CTRL+TAB
        ...
        // Note that if our window is collapsed we will end up with an inverted (~nullptr) clipping rectangle which is the correct behavior.
...
static void ImGui::NavUpdate()
{
    ...
    // In very rare cases g.NavWindow may be nullptr (e.g. clearing focus after requesting an init request, which does happen when releasing Alt while clicking on void)
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            // A) Read the explanation below, B) Use the ImGuiDragDropFlags_SourceAllownullptrID flag, C) Swallow your programmer pride.
