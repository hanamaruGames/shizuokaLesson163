...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // AllowOverlap mode (rarely used) requires previous frame HoveredId to be nullptr or to match. This allows using patterns where a later submitted widget overlaps a previous one.
...
bool ImGui::TempInputTextScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    if (value_changed)
    {
        value_changed = DataTypeApplyOpFromText(data_buf, g.InputTextState.InitialTextA.Data, data_type, p_data, nullptr);
        if (value_changed)
            MarkItemEdited(id);
    }
    return value_changed;
...
void    ImGui::EndTabItem()
{
    ...
    if (tab_bar == nullptr)
    {
        IM_ASSERT(tab_bar != nullptr && "Needs to be called between BeginTabBar() and EndTabBar()!");
        return;
    }
